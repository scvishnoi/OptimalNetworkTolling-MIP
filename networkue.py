...
A=[]
pathOD=[]
FRANK_WOLFE_STEPSIZE_PRECISION = 1e-9
...
class Network1:
   ...
   def relativeGap(self):
      ...
      for i in self.link:
         tstt+=self.link[i].flow*self.link[i].cost
         sptt+=allOrNothing[i]*self.link[i].cost
   ...
   def FrankWolfeStepSize(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION):
      ...
      temp_assign=dict()
      for i in self.link:
         temp_assign[i]=self.link[i].flow
         
      while True:
         self.shiftFlows(targetFlows,la)
         f=0
         for i in self.link:
            f+=self.link[i].cost*(targetFlows[i]-temp_assign[i])
            self.link[i].flow=temp_assign[i] 
         if(f==0):
            break
         if f>0:
            start=a
   ...
   def NewtonStepSize(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION):
      ...
      while True:
         ...
         df=0
         for i in self.link:
           difference=(targetFlows[i]-temp_assign[i])
           f+=self.link[i].cost*difference
           df+=self.link[i].freeFlowTime*self.link[i].alpha*self.link[i].beta* pow((self.link[i].flow/self.link[i].capacity), self.link[i].beta-1)*pow(difference,2)/(self.link[i].capacity)
           self.link[i].flow=temp_assign[i] 
         templa= la-f/df
         if templa<0:
            templa=0
   ...
   def userEquilibrium(self, stepSizeRule = 'MSA',
      ...
      while iteration < maxIterations:
         ...
         print("%i,%s,%f" % (iteration,time.time()-start_time, gap))
         
         if gap < targetGap:
            break
         targetFlows = self.allOrNothing()
         if stepSizeRule == 'FW':
            stepSize = self.NewtonStepSize(targetFlows)
