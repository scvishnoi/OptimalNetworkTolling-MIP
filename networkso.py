...
FRANK_WOLFE_STEPSIZE_PRECISION = 10E-9
pathOD = []
...
class Network:
   ...
   def printAllPathsUtil(self, u, d, visited): 
      ...
      if u ==d: 
         #print (pathOD)
         b=pathOD.copy()
         self.MASTER.append(b)
         
                        
      else: 
          # If current vertex is not destination 
          #Recur for all the vertices adjacent to this vertex 
          for i in self.node[u].forwardStar: 
              nextnode=self.link[i].head
              if visited[nextnode]==False:
                 self.printAllPathsUtil(nextnode, d, visited)
   ...
   def relativeGap(self):
      ...
      TSTT=0
      SPTT=0
      splink= self.allOrNothing()
      
      for i in self.link:
          TSTT= TSTT+ self.link[i].flow*self.link[i].cost
          
      for j in self.link:
          SPTT = SPTT + splink[j]*self.link[j].cost
   ...
   def averageExcessCost(self):
      ...
      for i in self.link:
          vcr= self.link[i].flow/self.link[i].capacity
          toll=self.link[i].flow*self.link[i].freeFlowTime*self.link[i].alpha*(self.link[i].beta)*pow(vcr, (self.link[i].beta-1))/self.link[i].capacity
          TSTT= TSTT+ self.link[i].flow*(self.link[i].cost+ toll)
          
      for i in self.link:
          vcr= self.link[i].flow/self.link[i].capacity
          toll=self.link[i].flow*self.link[i].freeFlowTime*self.link[i].alpha*(self.link[i].beta)*pow(vcr, (self.link[i].beta-1))/self.link[i].capacity
          SPTT = SPTT + splink[i]*(self.link[i].cost+toll)
        
      for k in self.ODpair:
          demand= demand+ self.ODpair[k].demand
      
      aec = (TSTT-SPTT)/demand
      return aec
   ...
   def shiftFlows(self, targetFlows, stepSize):
      ...
      *** IMPORTANT: After updating the flow on a link, you should call its
      ...
      """
      for i in self.link:
          self.link[i].flow= (1-stepSize)*self.link[i].flow+ stepSize*targetFlows[i]
          self.link[i].updateCost()
          
   def Newtons(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION):
            
      x1=1
      stepsize=0.5
            
      flowcopy= copy.deepcopy(self.link)
      while abs(x1-stepsize) > precision:
          s=0
          ff=0
          stepsize = x1      
          for i in self.link:
              difference= targetFlows[i]-flowcopy[i].flow
              self.link[i].flow= (1-stepsize)*flowcopy[i].flow+ stepsize*targetFlows[i]
              self.link[i].updateCost()
              s= s+ self.link[i].cost*difference 
              ff= ff+ self.link[i].freeFlowTime*self.link[i].alpha*self.link[i].beta* pow((self.link[i].flow/self.link[i].capacity), self.link[i].beta-1)*pow(difference,2)/(self.link[i].capacity)
          x1= stepsize-s/ff
          if x1<0:
              x1=0
          if x1>1:
      ...
      for j in self.link:
          self.link[j].flow=flowcopy[j].flow
   ...
   def FrankWolfeStepSize(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION):
      ...
      flowcopy= copy.deepcopy(self.link)
      while abs((a+b)/2 - stepsize) > precision:
          s=0
          stepsize = (a+b)/2      
          for i in self.link:
              difference= targetFlows[i]-flowcopy[i].flow
              ...
              vcr= self.link[i].flow/self.link[i].capacity
              toll=self.link[i].flow*self.link[i].freeFlowTime*self.link[i].alpha*(self.link[i].beta)*pow(vcr, (self.link[i].beta-1))/self.link[i].capacity
              s= s+ self.link[i].cost*difference +toll*difference     
   ...
   def userEquilibrium(self, stepSizeRule = 'MSA',
                          maxIterations = 10,
                          targetGap = 1e-6, 
                          gapFunction = relativeGap, starttime=0):
      """
      ...
      while iteration < maxIterations:
         ...
         tost=0
         for k in self.link:
             tost+=self.link[k].flow*self.link[k].cost
         #if(iteration%10==0):
         print("Iteration %d: gap %f: time %f" % (iteration, gap, tost))
         if gap < targetGap:
            break
         #if (timeit.default_timer()-starttime)>1200.00000000:
             #break
      ...
      *** and the shortest path tree should reflect this.
      ...
      while len(scanList) > 0:
         ...
         for hi in self.node[i].reverseStar:
            ...
            if h < self.firstThroughNode and h != origin:
               continue
            vcr= self.link[hi].flow/self.link[hi].capacity
            toll=self.link[hi].flow*self.link[hi].freeFlowTime*self.link[hi].alpha*(self.link[hi].beta)*pow(vcr, (self.link[hi].beta-1))/self.link[hi].capacity
            tempCost = cost[h] + self.link[hi].cost+toll
            if tempCost < cost[i]:
               cost[i] = tempCost
               backlink[i] = hi
               labelChanged = True
